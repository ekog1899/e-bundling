...
class Config extends CI_Controller {
    ...
    public function passwd($act=null){
            ...
            if ( $act == null )
            {
				...
				$data['content'] = $this->load->view("f_password",$data_content,true);
                $this->load->view("template",$data);
            }
            else{
                if ( $this->input->post('p2') != $this->input->post('p3') )
                {
                 die("Password tidak sama");
                }    // check pass lama
                $sql = "select * from t_user where id='".$this->session->userdata('user_id')."' and password=md5(".$this->input->post('p1').")";
                ...
                if ($query->num_rows() == 0) {
                    ...
                    $data['password']=md5($this->input->post('p2'));
                    #echo $data['password'];
