		...
		<div class="tab-pane fade" id="pemeriksa" role="tabpanel" aria-labelledby="sidang">
			...
			<table class="table_form" style="font-size:14px;width:100%;" border="0">
				...
				<tbody>
					...
					<!--raehman_2024_10_15_tambahan_BAS_Berkas-->
						<tr class=""> <td  colspan="2"> &nbsp;</td> </tr>
						<tr class="">
							<td td class="first-colum form-group bg-primary text-white">
								Tgl Penyerahan Berkas </td>
							<td>
								<?php
								if ($this->session->userdata('user_jabatan_id') == '41') { 
								?>
									<input type="date" value="<?php echo $cektim['tgl_serah_terima_berkas']; ?>" style="width: 200px;    background-color: #fff; background-clip: padding-box; border: 1px solid #d1d3e2; border-radius: .35rem;font-size: .875rem; line-height: 1.5;" onchange="edit_tgl_serah_terima_berkas(<?php echo $banding_detil[0]['perkara_id']; ?>,<?php echo $idpn; ?>,this.value)">
								<?php
									}
								?>
								 
								<div class="float-right">
									<a target="_blank" class="btn btn-success btn-sm" href="<?php echo base_url('c_cetak/cetak_serah_terima_berkas/'.$idpn.'/'.$banding_detil[0]['perkara_id'])?>" role="button">Cetak</a>
								</div>
								<script type="text/javascript">
									function edit_tgl_serah_terima_berkas(perkara_id, idpn, tgl_serah_terima_berkas){
									    var xhr = new XMLHttpRequest();
									    xhr.open("POST","<?php echo base_url('c_cetak/update_tgl_serah_terima_berkas')?>", true); 
									    ...
									    xhr.send("perkara_id="+perkara_id+"&idpn="+idpn+"&tgl_serah_terima_berkas="+tgl_serah_terima_berkas); 
									}
								</script>
							</td>
						</tr>
