...
class Monitoring_model extends CI_Model {
    ...
    function banding_belum_register($tahun){
        // $sql = "SELECT idpn, perkara_id, nomor_perkara_pa, tgl_putusan_pa, pemohon_banding, permohonan_banding, status_perkara_banding, nomor_perkara_banding, tanggal_pendaftaran_banding,YEAR(permohonan_banding) AS yr FROM sipp_perkara_banding  WHERE tanggal_pendaftaran_banding IS NULL AND YEAR(permohonan_banding) =$tahun";
        $sql = "SELECT a.*,b.blm_validasi,DATEDIFF(NOW(),permohonan_banding) jml_hari FROM sipp_perkara_banding a 
        LEFT JOIN
        (SELECT idpn,perkara_id,SUM(blm_validasi) blm_validasi 
            FROM (SELECT idpn,perkara_id,COUNT(1) blm_validasi FROM dok_perkara_banding
                    WHERE val_edoc IS NULL
                    AND jenis_edoc NOT IN (SELECT jenis_edoc FROM ref_jenis_edoc WHERE bundel='c')
                    AND file_edoc IS NOT NULL
                    GROUP BY idpn,perkara_id
                    ...
                    AND YEAR(permohonan_banding)=$tahun
                    ORDER BY blm_validasi DESC";
        return $this->db->query($sql)->result_array();
    }

    function belum_ada_tim_pemeriksa($tahun){
        $sql="SELECT a.*,b.blm_validasi,DATEDIFF(NOW(),permohonan_banding) jml_hari FROM sipp_perkara_banding a 
        LEFT JOIN
        (SELECT idpn,perkara_id,SUM(blm_validasi) blm_validasi 
            FROM (SELECT idpn,perkara_id,COUNT(1) blm_validasi FROM dok_perkara_banding
                    ...
                    LEFT JOIN `db_ebundling`.`tim_pemeriksa_pradaftar` 
        ON (a.`perkara_id` = `tim_pemeriksa_pradaftar`.`perkara_id`)
                    WHERE nomor_perkara_banding IS NULL OR nomor_perkara_banding = ''
                    AND status_perkara_banding NOT LIKE '%cabut%'
                    AND (YEAR(permohonan_banding)=$tahun-1 or YEAR(permohonan_banding)=$tahun)
                    AND `tim_pemeriksa_pradaftar`.`perkara_id` IS NULL 
                    ORDER BY blm_validasi DESC";
		return $this->db->query($sql)->result_array();
	}
	
    function perkara_ban_putus(){
        $sql = "CALL sp_banding_putus();";
        return $this->db->query($sql)->result_array();
    }
}
?>
