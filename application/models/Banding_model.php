...
class Banding_model extends CI_Model
{
	...
	function view_where_cetak($perkara_id, $idpn)
	{
		...
		$sql = "SELECT A.*,B.`nip`,B.`nama`,
			C.`nip` AS nip_tim1,C.`nama` AS nama_tim1,
			D.`nip` AS nip_tim2,D.`nama` AS nama_tim2,
			E.`nip` AS nip_tim_panitera,E.`nama` AS nama_panitera
			FROM `tim_pemeriksa_pradaftar` A
			LEFT JOIN `t_user_pta` B ON A.`id_ketua_tim` = B.`id`
			LEFT JOIN `t_user_pta` C ON A.`id_anggota_tim_1` = C.`id`
			LEFT JOIN `t_user_pta` D ON A.`id_anggota_tim_2` = D.`id`
			LEFT JOIN `t_user_pta` E ON A.`id_panitera_tim` = E.`id`
	...
	public function hapuscatatan($table, $where)
	{
	    ...
	    // use for banding kasasi satker 
    function banding_kasasi_satker_tahun($tahun, $satker){
         $sql = "SELECT satnama,satsing,banding,kasasi_skrg,kasasi_depan FROM satker aa LEFT JOIN
                ...
                WHERE satsing ='".$satker."'
                ORDER BY kode2 ASC";
        return $this->db->query($sql)->result_array();
    }

	// use for ist data banding
   ...
   function list_kasasi_skrg($satker,$tahun){
	...
	// use for list perkara kasasi mendatang
	...
	function insert_edoc_kasasi($data)
	{
		...
		$this->db->where('perkara_id', $data['perkara_id']);
		$this->db->where('jenis_edoc', $data['jenis_edoc']);
		$this->db->select('file_doc');
		$old_file = $this->db->get('dok_perkara_kasasi')->row();
		if (count($old_file) > 0)
			if (@file_exists('./uploads/edoc/' . $old_file->file_doc)) {
				unlink('./uploads/edoc/' . $old_file->file_doc);
			}
		$this->db->where('idpn', $data['idpn']);
		$this->db->where('perkara_id', $data['perkara_id']);
		$this->db->where('jenis_edoc', $data['jenis_edoc']);
		$this->db->delete('dok_perkara_kasasi');

		if ($data['file_size'] > 0) {
			#echo "inserted";
			$this->db->insert('dok_perkara_kasasi', $data);
		}
	}
}
