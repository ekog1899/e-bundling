...
class M_akses extends CI_Model
{
    ...
    function __construct()
    {
        ...
        //$this->database=$this->config->item('db_ebundling');
    ...
    function check_user_pta()
    {
        ...
        $this->db->limit(1);
        $sql = "select nama,nip, grup, case when kode_jabatan in (1,2,3,4,41,44) then TRUE else FALSE end is_verifikator,kode_jabatan from t_user_pta where username='" . $this->input->post('username') . "' and password='" . md5($this->input->post('password')) . "' limit 1";
    ...
    function chek_userpass()
    {
        ...
        $kweri_password = $this->db->query("select password,username,nip from sipp_user where nip='" . $this->input->post('username') . "'")->row();
        if (!empty($kweri_password->password)) {

            $password = $kweri_password->password;
            $username = $kweri_password->nip;
        } else {
    ...
    {
        $sql = "select * from t_user_pta where aktif ='1' and kode_jabatan in (1,2,3)";
        return $this->db->query($sql)->result_array();
    }

    function get_pp_banding()
    {
        $sql = "select * from t_user_pta where aktif ='1' and kode_jabatan in (4,41,42,44)";
        return $this->db->query($sql)->result_array();
    }
}
